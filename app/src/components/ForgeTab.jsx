      ...
      if (data.success && data.data && data.data.length > 0) {
        ...
        if (relevantVotes.length > 0) {
          ...
          if (rarityVote) {
            ...
            // Fetch user's ticket info from API
            try {
              const userResponse = await fetch(
                `https://memeforge-api-836651762884.asia-southeast1.run.app/api/users/${walletAddr}`
              );
              const userData = await userResponse.json();
              if (userData.success && userData.data) {
                setUserTickets(userData.data.weeklyTickets || 0);
                setVotingStreak(userData.data.streakDays || 0);
                // Calculate today's earned tickets (estimate based on rarity vote = 8-15 tickets)
                // We'll show the user's total weekly tickets instead
                setEarnedTickets(userData.data.weeklyTickets || 0);
              }
            } catch (userError) {
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-green-600 bg-opacity-20 border border-green-600 rounded-xl p-4">
                ...
                <h3 className="font-bold text-sm mb-1">Weekly Tickets</h3>
                <div className="text-xl text-green-300">{userTickets || earnedTickets}</div>
              </div>

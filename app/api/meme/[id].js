...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Schema.org structured data — Article type for AEO recognition
    const schemaData = {
    ...
    const breadcrumbData = {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 2, "name": "Meme Gallery", "item": `${SITE_URL}/#gallery` },
        { "@type": "ListItem", "position": 3, "name": meme.title || 'Meme', "item": canonicalUrl }
      ]
    };
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .content-section a { color: #06b6d4; text-decoration: none; }
...
<body>
  <div class="container">
    ...
    <section class="content-section">
      ...
      <p>This meme was generated by <strong>Memeya</strong>, the autonomous AI agent behind AI MemeForge, using ${style ? `a <strong>${style}</strong> art style and ` : ''}real-time crypto news${newsSource ? ` from <strong>${newsSource}</strong>` : ''}. It was created on <strong>${formatDate(createdAt)}</strong> as part of the daily 3-meme batch and received <strong>${totalVotes} community votes</strong>${yesVotes > 0 ? ` (${yesVotes} selections)` : ''}.</p>

      ${rarity ? `<p>After community voting concluded, this meme earned a final rarity of <strong>${escapeHtml(rarity)}</strong>. Rarity is determined by the ratio of votes a meme receives relative to the day's total — Common (&lt;25%), Uncommon (25-40%), Rare (40-55%), Epic (55-70%), and Legendary (&gt;70%).</p>` : ''}

      ${isWinner ? `<p>This meme was selected as the <strong>Daily Winner</strong> and was minted as a Metaplex Programmable NFT (pNFT) on Solana with metadata permanently stored on Arweave — making it one of only 365 memes minted per year.</p>` : ''}

      <h3>How AI MemeForge Works</h3>
      <p>Every day, AI generates 3 unique crypto memes from real-time news. The community votes for free — no gas fees, no tokens required. Each vote earns 8-15 lottery tickets. At 23:56 UTC, a weighted lottery selects one winner who claims the day's top-voted meme as a Solana pNFT.</p>

      <h3>Key Facts</h3>
      <ul>
        <li><strong>3 memes generated daily</strong> using Gemini and Grok AI from real-time crypto news</li>
        <li><strong>100% free to participate</strong> — no gas, no tokens, no fees to vote or enter the lottery</li>
        <li><strong>365 NFTs per year</strong> — one minted per day as a Metaplex pNFT on Solana</li>
        <li><strong>$Memeya token holders</strong> earn bonus tickets: floor(log10(tokens held)) extra per vote</li>
        <li><strong>Permanent storage</strong> — NFT metadata and images stored on Arweave</li>
      </ul>

      <p>Want to vote on today's memes? <a href="${SITE_URL}">Connect your Solana wallet</a> and start earning lottery tickets. Follow <a href="https://x.com/AiMemeForgeIO">@AiMemeForgeIO on X</a> or join the <a href="https://t.me/MemeyaOfficialCommunity">Telegram community</a> for daily updates.</p>
    </section>
    ...
    <footer class="footer">
      ...
      <p style="margin-top: 16px; font-size: 12px; color: #6b7280;">
        © 2026 AI MemeForge. Powered by AI & Solana.
      </p>
    </footer>
  </div>
</body>

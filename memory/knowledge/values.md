...
- **[2026-03-01 更新]** Release 完整流程：
  0) git pull 確保與 GitHub 同步
  1) 檢查 git status (確認無未提交變更)
  2) 檢查當前版號 (e.g., git describe --tags --abbrev=0)
  3) 如有變更 → git_commit
  4) 用 `git log --oneline $(git describe --tags --abbrev=0)..HEAD` 列出所有新 commits
  5) git_release (推送 + 標籤，version="auto")
  6) 確認 GitHub release page 同步
  ...
  setupListeners() {
    this.bot.on('message', (msg) => {
      ...
      if (text === '#chatmode') {
        ...
        this.bot.sendMessage(this.chatId, '🔄 收到切換指令，正在結束手邊工作並切換到 Chat Mode...');
        console.log('[TG] Mode switch to chat requested');
        return;
      }

      // === #devmode — Switch to Dev Mode ===
      if (text === '#devmode') {
        this.mustQueue.push({
          type: 'mode_switch',
          command: 'dev',
        ...
        this.bot.sendMessage(this.chatId, '🔄 收到切換指令，正在切換到 Dev Mode...');
        console.log('[TG] Mode switch to dev requested');
        return;
      }

# MemeForge — Technical Design Document

> 系統架構、API 規格、資料模型、部署配置、實作規劃與開發進度追蹤

*最後更新: 2026-03-07*

---

## 1. 系統總覽

MemeForge 的完整技術系統支撐以下產品循環（詳見 product.md）：

```
AI 生成梗圖 → 社群投票 → 選出每日贏家 → 每日抽獎選出擁有者 → Claim 鑄造 NFT
```

**每日排程 (UTC):**
| 時間 | Job | 動作 |
|------|-----|------|
| 23:50 | end_voting | 結算投票、選出贏家梗圖、計算稀有度 |
| 23:55 | lottery_draw | 加權隨機抽選擁有者、參與者 tickets 歸零 |
| 00:00 | daily_cycle | AI 生成 3 張新梗圖、開始新投票期 |

---

## 2. 系統架構

### 2.1 架構總覽

```
┌─────────────────────────────────────────────────────────────────┐
│                     Frontend (Vercel)                           │
│                https://solana-hacker.vercel.app                 │
│                                                                 │
│  React + Vite + Tailwind CSS + Privy SDK + Solana Web3.js        │
│                                                                 │
│  READ: Firebase SDK 直連 (即時同步 onSnapshot)                   │
│  WRITE: Cloud Run API (驗證 + 防刷)                              │
└─────────────────────────────────────────────────────────────────┘
                │ READ                    │ WRITE
                ▼                         ▼
┌──────────────────────┐    ┌──────────────────────────────────────┐
│     Firestore        │◄──►│         Cloud Run API (GCP)          │
│  (即時資料庫)         │    │  memeforge-api-836651762884          │
│                      │    │  asia-southeast1                     │
│ users/{wallet}       │    ├──────────────────────────────────────┤
│ memes/{id}           │    │ /api/memes/today          GET        │
│ votes/{id}           │    │ /api/memes/generate-daily POST       │
│ voting_periods/{id}  │    │ /api/voting/vote          POST       │
│ voting_progress/{id} │    │ /api/stats                GET        │
│ platform_stats/      │    │ /api/users/{wallet}       GET        │
│ scheduler_logs/{id}  │    │ /api/scheduler/trigger/*  POST       │
│ hall_of_memes/{id}   │    │ /api/og/:memeId           GET        │
└──────────────────────┘    └──────────────────────────────────────┘
                                       │
                       ┌───────────────┼───────────────┐
                       ▼               ▼               ▼
              ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
              │  GCS Bucket  │ │  Gemini AI   │ │   Grok API   │
              │  memeforge-  │ │  梗圖生成     │ │  新聞分析     │
              │  images-web3ai│ │              │ │  趨勢監測     │
              └──────────────┘ └──────────────┘ └──────────────┘
```

### 2.2 讀寫分離設計

| 操作類型 | 通道 | 原因 |
|---------|------|------|
| 讀取梗圖 | Firebase SDK 直連 | 即時同步 `onSnapshot`、低延遲 |
| 讀取投票統計 | Firebase SDK 直連 | 多用戶即時看到變化 |
| 提交投票 | Cloud Run API | 驗證錢包簽名、防重複/刷票 |
| 生成梗圖 | Cloud Run API | API Key 不暴露給前端 |
| 排程任務 | Cloud Scheduler → Cloud Run | 外部觸發，可靠執行 |

### 2.3 技術棧

| 層級 | 技術 | 說明 |
|------|------|------|
| Frontend | React + Vite | SPA，Vercel 部署 |
| Styling | Tailwind CSS | 響應式設計 |
| Auth & Wallet | Privy SDK (`@privy-io/react-auth`) | Google 登入 (embedded wallet) + OKX/Phantom 優先 + 其他 Solana 錢包 |
| Backend | Node.js + Express | API 服務，Cloud Run 部署 |
| Database | Firebase/Firestore | 即時資料庫 |
| Storage | Google Cloud Storage | 梗圖圖片 (Uniform Bucket-Level Access) |
| AI | Gemini 3 Pro Image | 梗圖圖片生成 (多模型之一) |
| AI | Grok Imagine Image Pro | 梗圖圖片生成 (多模型之一, xAI API) |
| AI | Gemini 2.5 Flash | 梗圖文字生成 (prompt, title, tags, description) |
| AI | Grok API (xAI) | 新聞分析 |
| Scheduler | GCP Cloud Scheduler | 外部 cron 排程 |
| CDN | Vercel Edge | 前端快速分發 |

---

## 3. 專案結構

```
/home/projects/solanahacker/
├── agent/                    # Agent 程式碼
│   ├── .env                  # Agent 環境變數 (Grok, Gemini, X API, Tapestry)
│   ├── main.js               # Agent 主入口 (heartbeat, mode switching)
│   ├── chat-mode.js           # Heartbeat: maybePostToX + maybeAnnounceWinners + maybeCheckOwnerMentions + maybePostToMoltbook + maybeMoltbookEngage + maybeDistillMemory
│   ├── telegram-bridge.js       # Telegram bridge (operator commands: #news, #status)
│   ├── tg-community.js        # TG Community Bot (@memeya_bot in @MemeyaOfficialCommunity)
│   ├── dashboard-server.js    # Dashboard HTTP server (port 8090)
│   ├── dashboard.html         # Agent 主 dashboard
│   ├── memeya-dashboard.html  # Memeya Dashboard (tabs: X, Moltbook, Timers, System)
│   └── skills/
│       ├── x_twitter/
│       │   ├── index.js       # X posting: generateTweet, autoPost, extractBannedPatterns, buildSystemPrompt
│       │   └── x-context.js   # Context gathering + topic rotation + comment patrol
│       └── moltbook/
│           ├── index.js       # Moltbook skill: autoPostMemes, engage, postIntroduction, ensureSetup
│           ├── moltbook-client.js  # Moltbook API client (auth, rate limits, verification solver)
│           └── skill.md       # Skill metadata for registry
├── app/
│   ├── src/                  # Frontend (React + Vite)
│   │   ├── components/       # UI 組件 (含 AgentPage.jsx)
│   │   ├── hooks/            # React Hooks
│   │   └── services/         # Firebase + API 服務層
│   ├── backend/              # Backend (Express + Firebase Admin)
│   │   ├── server.js         # 主入口
│   │   ├── routes/           # API 路由
│   │   ├── services/         # 業務邏輯
│   │   ├── controllers/      # 控制器
│   │   └── .env              # Backend 環境變數
│   ├── public/               # 靜態資源 (含 agent.json)
│   └── index.html            # OG meta tags
├── docs/
│   ├── product.md            # 產品文件
│   ├── TDD.md                # 本文件 (技術設計)
│   └── _transient/           # 過渡性文件
└── memory/                   # Agent 記憶
    ├── journal/memeya/       # 短期記憶 (每日日記)
    └── knowledge/            # 核心價值觀 + 長期記憶
```

### 關鍵 Frontend 檔案

| 檔案 | 用途 |
|------|------|
| `src/components/PrivyAuthProvider.jsx` | Privy SDK 初始化 + 認證 Provider |
| `src/hooks/useAuth.js` | 統一認證 Hook (Privy login/logout, wallet, exportWallet) |
| `src/services/firebase.js` | Firebase Client SDK + 即時監聽 |
| `src/services/memeService.js` | API 服務層 (Cloud Run 呼叫) |
| `src/services/solanaService.js` | $Memeya token 餘額查詢 (via backend proxy) + 顯示格式化 |
| `src/hooks/useFirebase.js` | useTodayMemes, useVoteStats 等 Hooks |
| `src/components/Dashboard.jsx` | 主 Dashboard (設定選單, 私鑰匯出, tabs) |
| `src/components/ForgeTab.jsx` | 主投票頁面 |
| `src/components/GalleryTab.jsx` | Hall of Memes 歷史展示 (Top Voted 篩選 + #1 badge) |
| `src/components/MemeModal.jsx` | 梗圖大圖 + 投票 + 分享按鈕 |
| `src/components/ModalOverlay.jsx` | Modal 基礎組件 (React Portal) |
| `src/components/AgentPage.jsx` | Memeya Agent 公開個人檔案頁面 |

### 關鍵 Backend 檔案

| 檔案 | 用途 |
|------|------|
| `backend/server.js` | Express 主入口、路由註冊 |
| `backend/services/schedulerService.js` | 排程任務核心邏輯 (end_voting, daily_cycle, lottery) |
| `backend/controllers/memeController.js` | 梗圖 CRUD (getTodaysMemes, generateDailyMemes, 多模型隨機選擇) |
| `backend/services/grokImageService.js` | Grok 圖片生成 (xAI Images API, b64_json → GCS) |
| `backend/routes/scheduler.js` | 排程 API 路由 (/api/scheduler/trigger/*) |
| `backend/routes/og.js` | OG Card 動態圖片生成 |
| `backend/routes/voting.js` | 投票 API 路由 |
| `backend/services/solanaService.js` | $Memeya token 餘額查詢 (直接 JSON-RPC, Token-2022 相容) + bonus 計算 |
| `backend/routes/users.js` | 用戶資料 API + $Memeya 餘額 proxy |

---

## 4. API 規格

### 4.1 梗圖 API

**`GET /api/memes/today`** — 取得今日梗圖
```javascript
// Response
{
  success: true,
  memes: [{
    id: 'meme_1739836800000_0',
    title: 'When Solana Hits $200',
    imageUrl: 'https://storage.googleapis.com/memeforge-images-web3ai/memes/xxx.png',
    generatedAt: '2026-02-18T00:00:00.000Z',
    status: 'voting_active',
    votes: { selection: { yes: 42, no: 10 } },
    sentiment: 'Bullish',
    tags: ['solana', 'price'],
    style: 'Classic Oil Painting'
  }],
  count: 3
}
```

**`POST /api/memes/generate-daily`** — 生成每日梗圖 (Scheduler 觸發)

**`GET /api/memes/generate-price`** — 取得付費生成即時價格
```javascript
// Response
{
  success: true,
  prices: {
    SOL: { amount: 0.00075, usdValue: 0.10 },
    MEMEYA: { amount: 125000, usdValue: 0.08, discount: '20%' },
    USDC: { amount: 0.10, usdValue: 0.10 }
  }
}
```

**`POST /api/memes/generate-solana`** — 付費梗圖生成 (Solana 鏈上支付)
```javascript
// Request
{
  topic: 'Solana flipping Ethereum',
  txSignature: '5xyz...abc',      // Solana 交易簽名
  paymentToken: 'SOL' | 'MEMEYA' | 'USDC'
}
// Response
{ success: true, meme: { id, title, imageUrl, ... } }
```

**`GET /api/memes/my-creations`** — 查詢用戶已購買梗圖
```javascript
// Query: ?wallet=ABC123...xyz&limit=20
// Response
{ success: true, memes: [{ id, title, imageUrl, topic, createdAt }] }
```

**`DELETE /api/memes/:id`** — 刪除梗圖 (驗證 solana_orders 擁有權)
```javascript
// Request body
{ wallet: 'ABC123...xyz' }
// Response
{ success: true }
```

### 4.2 投票 API

**`POST /api/voting/vote`** — 提交投票
```javascript
// Request
{
  memeId: 'meme_xxx',
  vote: 'yes',
  walletAddress: 'ABC123...xyz',
  signature: '...'  // 錢包簽名
}

// Response
{
  success: true,
  ticketsAwarded: 12,
  streakDays: 5,
  message: 'Vote recorded! +12 tickets'
}
```

### 4.3 用戶 API

**`GET /api/users/{wallet}`** — 取得或建立用戶
```javascript
// Response
{
  walletAddress: 'ABC123...xyz',
  weeklyTickets: 42,
  totalTicketsAllTime: 350,
  streakDays: 5,
  lastVoteDate: '2026-02-18',
  lotteryOptIn: true
}
```

**`GET /api/users/{wallet}/memeya-balance`** — 取得 $Memeya token 餘額與 bonus
```javascript
// Response
{
  success: true,
  data: {
    balance: 1167965.158142,  // human-readable (raw / 10^6)
    bonus: 6                  // floor(log10(balance)), 0 if < 10
  }
}
```

> 後端透過直接 JSON-RPC 呼叫 Helius (SOLANA_RPC_URL) 查詢 Token-2022 帳戶。前端快取 10 分鐘 (localStorage)。

**`GET /api/users/list/leaderboard`** — Ticket 排行榜

### 4.4 排程 API

**`POST /api/scheduler/trigger/{task}`** — Cloud Scheduler 觸發端點

| Task | 說明 |
|------|------|
| `end_voting` | 結算投票、選出贏家 |
| `daily_cycle` | AI 生成新梗圖 + 開始投票 |
| `lottery_draw` | 每日抽獎選出擁有者 |

**`GET /api/scheduler/logs`** — 排程執行日誌

### 4.5 其他 API

| 端點 | 方法 | 說明 |
|------|------|------|
| `/api/stats` | GET | 平台統計 |
| `/api/stats/increment-voters` | POST | 增加投票者計數 |
| `/api/og/:memeId` | GET | 動態 OG Card 圖片 |
| `/health` | GET | 健康檢查 |
| `/api/lottery/ticket-distribution/:drawId` | GET | 指定抽獎的 ticket 分佈 (前 10 名 + 登入用戶排名) |

---

## 5. Firestore Schema

### 5.1 Collections 概覽

| Collection | Document ID | 用途 |
|------------|-------------|------|
| `users` | `{walletAddress}` | 用戶資料、tickets、streak |
| `memes` | `{memeId}` | 梗圖資料、投票數、擁有者 |
| `votes` | `{voteId}` | 投票記錄 |
| `voting_periods` | `{periodId}` | 投票期間追蹤 |
| `voting_progress` | `{walletAddress}` | 用戶投票進度 |
| `platform_stats` | `current` | 平台統計 |
| `scheduler_logs` | `{logId}` | 排程執行日誌 |
| `hall_of_memes` | `{date}` | 歷史贏家 |
| `solana_orders` | `{txSignature}` | Solana 付費梗圖訂單 |

### 5.2 Document Schemas

**users/{walletAddress}**
```javascript
{
  walletAddress: 'ABC123...xyz',
  weeklyTickets: 42,           // 當前持有 tickets (抽獎後歸零)
  totalTicketsAllTime: 350,    // 歷史總 tickets
  streakDays: 5,               // 連續投票天數
  lastVoteDate: '2026-02-18',  // 最後投票日 (YYYY-MM-DD)
  lotteryOptIn: true,          // 預設 true，用戶可 toggle
  nftWins: [                   // 中獎記錄
    { memeId: 'meme_xxx', title: '...', selectedAt: '...', claimed: false }
  ],
  createdAt: Timestamp,
  updatedAt: Timestamp
}
```

**memes/{memeId}**
```javascript
{
  id: 'meme_1739836800000_0',
  type: 'daily',
  status: 'active' | 'voting_active' | 'voting_completed' | 'winner' | 'minted',
  generatedAt: '2026-02-18T00:00:00.000Z',
  title: 'When Solana Hits $200',
  description: 'AI-generated from CoinDesk news',
  imageUrl: 'https://storage.googleapis.com/memeforge-images-web3ai/memes/xxx.png',
  newsSource: 'CoinDesk',
  sentiment: 'Bullish',
  tags: ['solana', 'price', 'moon'],
  style: 'cyberpunk_neon',          // V3: art style ID (was: style mode string)
  votes: { selection: { yes: 42, no: 10 } },
  metadata: {
    aiModel: 'gemini-3-pro-image-preview' | 'grok-imagine-image-pro',
    imageGenerated: true,
    // V3 generation pipeline fields
    styleMode: 'cyberpunk_neon',     // backward-compat alias of artStyleId
    artStyleId: 'cyberpunk_neon',    // V1 art style ID
    artStyleName: 'Cyberpunk Neon',  // V1 art style display name
    artStyleNftTier: 'premium',      // 'standard' | 'premium' | 'rare'
    generationMode: 'template',      // 'template' (Mode A) | 'original' (Mode B)
    templateId: 'drake',             // null for Mode B
    templateName: 'Drake Approve/Reject', // null for Mode B
    archetype: 'comparison',         // null for Mode B
    strategyId: 'ironic_flex',
    strategyName: 'Ironic Flex',
    narrativeId: 'fomo_degen',
    narrativeName: 'FOMO Degen',
    narrativePhrase: '...',
    narrativeEmotion: 'greedy',
    narrativeRole: 'degen trader',
    memeIdea: { caption, caption_slots, visual_description, emotion, twist, event_angle },
    qualityScore: 88,
    qualityPass: true,
    qualityScores: { template_familiarity, caption_punchiness, crypto_nativeness, immediacy, twist_strength, originality_score },
    originalityScore: 4,
  },
  nftOwner: {                  // lottery_draw 設定
    walletAddress: 'ABC123...xyz',
    selectedAt: '2026-02-19T00:00:00Z',
    claimTxSignature: null,    // Phase 2: Claim 後填入
    claimedAt: null,           // Phase 2: Claim 後填入
    mintAddress: null           // Phase 2: NFT mint address
  }
}
```

**votes/{voteId}**
```javascript
{
  memeId: 'meme_xxx',
  walletAddress: 'ABC123...xyz',
  vote: 'yes',
  timestamp: '2026-02-18T10:30:00.000Z'
}
```

**platform_stats/current**
```javascript
{
  weeklyVoters: 0,
  totalVotersAllTime: 0,
  lastUpdated: Timestamp
}
```

**solana_orders/{txSignature}**
```javascript
{
  txSignature: '5xyz...abc',          // Solana 交易簽名 (document ID)
  sender: 'ABC123...xyz',             // 付款錢包地址
  paymentToken: 'SOL' | 'MEMEYA' | 'USDC',
  status: 'verified' | 'completed' | 'failed' | 'rate_limited' | 'deleted',
  amount: 0.00075,                    // 支付金額 (原生單位)
  memeId: 'meme_xxx',                 // 生成的梗圖 ID
  topic: 'Solana flipping Ethereum',  // 用戶輸入主題
  completedAt: Timestamp,             // 生成完成時間
  deletedAt: Timestamp                // 軟刪除時間 (status='deleted' 時設定)
}
```

### 5.3 Firestore 索引

| Collection | Fields | 排序 | 用途 |
|-----------|--------|------|------|
| `memes` | status, type, generatedAt | ↑, ↑, ↓ | 查詢今日活躍梗圖 |

Index 建立: [Firebase Console](https://console.firebase.google.com/project/web3ai-469609/firestore/indexes)

### 5.4 Meme 狀態機

```
active → voting_active → voting_completed → winner → minted
                                              ↑           ↑
                                        lottery_draw   NFT claim
```

- `active`: 剛生成，等待投票開始
- `voting_active`: 投票進行中
- `voting_completed`: 投票結算完成
- `winner`: 抽獎贏家已選出，等待 Claim
- `minted`: NFT 已鑄造 (終態，拒絕後續操作)

---

## 5.5 Meme Generation Pipeline (V1 → V2 → V3)

### 世代對照

| | V1 (~Mar 1) | V2 (Mar 2) | V3 (Mar 3+) |
|---|---|---|---|
| **視覺系統** | 10 種 V1 藝術風格，每張獨特 | 硬編碼 `meme_native` (MS Paint 風) | V1 藝術風格 × V2 模板佈局 |
| **構圖方式** | AI 自由構圖 | 15 種模板格式固定佈局 | Mode A 模板 + Mode B 自由構圖 |
| **文案生成** | 自由文字，無字數限制 | 欄位制 ≤6 字/欄，slot_limits | 同 V2 規則 (兩種模式都遵守) |
| **喜劇層** | 無 | 策略 (comedy angle) + 敘事 (trader psychology) | 同 V2 |
| **品質管控** | 無 | LLM 評分 ≥82/100 + 代碼驗證 + 最多 2 次重試 | 同 V2 |
| **防重複** | 7 天主題避重 | 模板 14 天封鎖 + 原型 3/7 天冷卻 + 策略/敘事輪轉 | 同 V2 + 藝術風格 7 天不重複 |
| **代表作** | "The BITCOIN Bounce", "Vitaliks Big Brain" | 精準笑話但視覺單調 | 兩者結合 |

### V3 兩種生成模式

**Mode A — Template + Art Style** (每日 2 張, meme 1-2)

```
selectTemplate → selectStrategy → selectNarrative
  → generateMemeIdea (V2 文案紀律, slot limits)
  → evaluateMemeIdea + retryMemeIdea (最多 2 次)
  → buildImagePrompt(template layout + V1 art style)
  → generateImage (Gemini/Grok 隨機)
```

同一模板 (如 "Drake Approve/Reject") 以不同藝術風格 (如 "Ink Wash Zen" vs "Cyberpunk Neon") 渲染，視覺完全不同。

**Mode B — Art-first Original** (每日 1 張, meme 3)

```
selectStrategy → selectNarrative
  → generateOriginalMemeIdea (V2 文案規則 + 自由視覺場景)
  → evaluateMemeIdea + retryOriginalMemeIdea (最多 2 次)
  → buildOriginalImagePrompt (V1 art style + visual_description 驅動構圖)
  → generateImage (Gemini/Grok 隨機)
```

跳過模板選擇。`visual_description` 是主要創作輸出 — AI 自由描述場景、角色、環境。文案仍遵守 ≤6 字、第一人稱、crypto-native 規則。

### V1 藝術風格清單

| ID | 名稱 | NFT 等級 |
|----|------|----------|
| `classic_2d` | Classic 2D Illustration | standard |
| `retro_pixel` | Retro Pixel Art | standard |
| `cyberpunk_neon` | Cyberpunk Neon | premium |
| `hyper_realism` | Hyper-Realism | premium |
| `abstract_glitch` | Abstract Glitch Art | rare |
| `classic_oil` | Classic Oil Painting | rare |
| `3d_clay` | 3D Clay Toy | standard |
| `ink_wash` | Ink Wash Zen | rare |
| `street_graffiti` | Street Graffiti | premium |
| `modern_flat` | Modern Flat Design | standard |

`selectArtStyle()` 排除 7 天內已使用的風格 ID。10 種風格 × 每日 3 張 = 每種風格約 3.3 天出現一次，無窗口耗盡風險。

### 防重複矩陣

| 維度 | Mode A (Template) | Mode B (Original) |
|------|-------------------|-------------------|
| 模板 | 14 天硬封鎖 | N/A |
| 原型 | 3/7 天軟冷卻 | N/A |
| 藝術風格 | 7 天不重複 | 7 天不重複 |
| 文案 | 策略+敘事輪轉 + 評估器 | 同左 |

同一批次內 3 張使用不同藝術風格 (`recentThemes.unshift` 即時更新)。

### 檔案架構

| 檔案 | 職責 |
|------|------|
| `memeIdeaService.js` | 模板選擇、藝術風格選擇、文案生成/重試、品質評估、圖片 prompt 建構 |
| `memeController.js` | 每日生成管線編排 (Mode A/B 分流)、Firestore 讀寫、metadata 組裝 |
| `memeV1Legacy.js` | V1 藝術風格資料 + `pickFreshStyle()` 選擇器 (純資料模組) |
| `memeStrategyService.js` | 喜劇策略選擇 (不受 V3 修改) |
| `memeNarrativeService.js` | 敘事/心理框架選擇 (不受 V3 修改) |
| `geminiService.js` | Gemini 圖像生成 (接收 prompt 字串，不受 V3 修改) |
| `grokImageService.js` | Grok 圖像生成 (接收 prompt 字串，不受 V3 修改) |

### Mint 資格

- **Mode A**: 模板 14 天內未重複 + 原型 7 天內未重複 (或所有原型已耗盡) → eligible
- **Mode B**: 原創作品，永遠 eligible

---

## 6. Cloud Scheduler

### 6.1 Jobs

| Job | Cron (UTC+8) | UTC | 端點 |
|-----|-------------|-----|------|
| `memeforge-end-voting` | 每日 7:50 AM | 23:50 | POST `/api/scheduler/trigger/end_voting` |
| `memeforge-lottery-draw` | 每日 7:55 AM | 23:55 | POST `/api/scheduler/trigger/lottery_draw` |
| `memeforge-daily-cycle` | 每日 8:00 AM | 00:00 | POST `/api/scheduler/trigger/daily_cycle` |

**執行順序**: end_voting (23:50) → lottery_draw (23:55) → daily_cycle (00:00) (5 分鐘間隔)

### 6.2 管理指令

```bash
# 列出排程
gcloud scheduler jobs list --location=asia-southeast1

# 手動觸發
gcloud scheduler jobs run memeforge-end-voting --location=asia-southeast1

# 查看日誌
curl https://memeforge-api-836651762884.asia-southeast1.run.app/api/scheduler/logs

# 新增 job
gcloud scheduler jobs create http memeforge-lottery-draw \
  --location=asia-southeast1 \
  --schedule="55 7 * * *" \
  --time-zone="Asia/Taipei" \
  --uri="https://memeforge-api-836651762884.asia-southeast1.run.app/api/scheduler/trigger/lottery_draw" \
  --http-method=POST \
  --headers="Content-Type=application/json"
```

---

## 7. 認證與安全

### 7.1 Privy 認證系統

MemeForge 使用 **Privy SDK** (`@privy-io/react-auth`) 處理用戶認證，支援多種登入方式：

**登入方式:**
| 方式 | 錢包類型 | 說明 |
|------|---------|------|
| Google OAuth | Embedded wallet (Privy 管理) | 零門檻，自動建立 Solana 錢包 |
| OKX Wallet | External wallet | 優先顯示 |
| Phantom | External wallet | 優先顯示 |
| 其他 Solana 錢包 | External wallet | 自動偵測 (detected_solana_wallets) |

**Embedded Wallet 私鑰匯出:**
- Google 登入用戶的 embedded wallet 由 Privy 管理
- 用戶可從 Dashboard 設定選單點擊「Export Private Key」
- 調用 `useExportWallet()` hook，開啟 Privy 安全 modal 讓用戶查看/複製私鑰
- 僅 embedded wallet 用戶可見此選項（外部錢包用戶不顯示）

**Frontend 認證 Hook (`useAuth.js`):**
```javascript
import { usePrivy, useWallets, useExportWallet } from '@privy-io/react-auth';

export function useAuth() {
  const { ready, authenticated, user, login, logout } = usePrivy();
  const { wallets } = useWallets();
  const { exportWallet } = useExportWallet();

  // Wallet address: 優先外部 Solana 錢包，fallback embedded wallet
  // hasEmbeddedWallet: 檢測 linkedAccounts 中 walletClientType === 'privy'
  // isGoogleUser: 檢測 linkedAccounts 中 type === 'google_oauth'

  return { ready, authenticated, walletAddress, shortAddress, walletName,
           isGoogleUser, hasEmbeddedWallet, exportWallet, login, logout, user };
}
```

**Privy 設定 (`PrivyAuthProvider.jsx`):**
- App ID: 環境變數 `VITE_PRIVY_APP_ID`
- 支援 Solana chain
- Login methods: `['google', 'wallet']`
- Embedded wallet: `createOnLogin: 'users-without-wallets'` (Google 用戶自動建立)

### 7.2 防刷機制

```javascript
app.post('/api/vote', async (req, res) => {
  const { memeId, vote, walletAddress, signature } = req.body;

  // 1. 驗證錢包簽名
  if (!verifyWalletSignature(walletAddress, signature))
    return res.status(401).json({ error: 'Invalid signature' });

  // 2. 防重複投票 (每錢包每梗圖一次)
  const existing = await db.collection('votes')
    .where('memeId', '==', memeId)
    .where('walletAddress', '==', walletAddress).get();
  if (!existing.empty)
    return res.status(400).json({ error: 'Already voted' });

  // 3. 頻率限制 (每錢包每分鐘最多 5 票)
  const recentVotes = await getRecentVotesCount(walletAddress, 60);
  if (recentVotes >= 5)
    return res.status(429).json({ error: 'Rate limited' });

  // 4. 寫入 Firestore
  await db.collection('votes').add({ memeId, vote, walletAddress, timestamp: new Date() });
  res.json({ success: true });
});
```

### 7.3 CORS 設定

```javascript
const corsOptions = {
  origin: [
    'https://solana-hacker.vercel.app',
    'https://solana-hacker-git-*.vercel.app', // Preview
    'http://localhost:5173'                    // Dev
  ],
  credentials: true
};
```

### 7.4 投票獎勵機制

```
投票完成 → base = 隨機 1-10 tickets
         → streakBonus = min(streakDays, 10)
         → tokenBonus = floor(log10(memeyaBalance)), 0 if < 10
         → ticketsEarned = base + streakBonus + tokenBonus
         → 更新 weeklyTickets + totalTicketsAllTime
         → 計算 streakDays (連續+1 或重置為1)
         → 記錄 lastVoteDate
```

---

## 8. 部署配置

### 8.1 環境分離

| 環境 | Frontend | Backend | Database |
|-----|----------|---------|----------|
| Development | http://165.22.136.40:5173 | localhost:3001 (DEV_MODE) | Mock Data |
| Production | https://solana-hacker.vercel.app | Cloud Run | Firestore |

### 8.2 環境變數

**Backend (Cloud Run)**
```bash
NODE_ENV=production
GEMINI_API_KEY=<gemini-api-key>
XAI_API_KEY=<xai-api-key>            # Grok image generation + news analysis
SOLANA_RPC_URL=<helius-mainnet-rpc>   # Helius RPC for $Memeya balance (Token-2022)
FIREBASE_PROJECT_ID=web3ai-469609
FIREBASE_CLIENT_EMAIL=firebase-adminsdk-fbsvc@web3ai-469609.iam.gserviceaccount.com
FIREBASE_PRIVATE_KEY=<firebase-private-key-pem-format>
```

**Frontend (Vercel)**
```bash
VITE_API_BASE_URL=https://memeforge-api-836651762884.asia-southeast1.run.app
VITE_PRIVY_APP_ID=<privy-app-id>
VITE_FIREBASE_API_KEY=<firebase-web-api-key>
VITE_FIREBASE_PROJECT_ID=web3ai-469609
VITE_FIREBASE_AUTH_DOMAIN=web3ai-469609.firebaseapp.com
VITE_FIREBASE_STORAGE_BUCKET=web3ai-469609.firebasestorage.app
```

**環境變數檔案位置:**

| 用途 | 路徑 |
|------|------|
| Backend | `app/backend/.env` + Cloud Run env vars |
| Frontend | `app/.env.local` + Vercel env vars |
| Agent | `agent/.env` |

### 8.3 GCS 儲存

- **Bucket**: `memeforge-images-web3ai`
- **路徑**: `memes/{filename}.png`
- **存取**: Uniform Bucket-Level Access (IAM public read)
- **URL**: `https://storage.googleapis.com/memeforge-images-web3ai/memes/xxx.png`

> 注意: 上傳時使用 `resumable: false`，不設定 `public: true` (與 Uniform Access 衝突)

### 8.4 部署流程

**Frontend (自動):**
```
git push origin main → Vercel 自動部署
```

**Backend (手動):**
```bash
ssh root@165.22.136.40
cd /home/projects/solanahacker/app/backend
gcloud run deploy memeforge-api --source . --region asia-southeast1 --allow-unauthenticated
gcloud run services update-traffic memeforge-api --region asia-southeast1 --to-latest
```

> 重要: 使用 `--update-env-vars` 而非 `--set-env-vars`，避免覆蓋既有環境變數。

**同步流程 (Local → GitHub):**
```bash
# 1. SCP 到 droplet
scp <local-file> root@165.22.136.40:/home/projects/solanahacker/<path>

# 2. SSH commit + push
ssh root@165.22.136.40 "cd /home/projects/solanahacker && git add -A && git commit -m 'message' && git push origin main"
```

### 8.5 健康檢查

```bash
curl https://memeforge-api-836651762884.asia-southeast1.run.app/health
# {"status":"healthy"}

curl https://memeforge-api-836651762884.asia-southeast1.run.app/api/memes/today
# {"success":true,"memes":[...],"count":3}
```

---

## 9. Agent Memeya — 自主 X 發文系統

### 9.1 架構總覽

```
┌──────────────────────────────────────────────────────────────────┐
│                    chat-mode.js (Heartbeat — 60min interval)      │
│                                                                   │
│  doHeartbeat()                                                    │
│    ├─ writeTimerState()       dashboard timer visibility          │
│    ├─ maybeCheckOwnerMentions() 15min timer, owner mention reply  │
│    ├─ maybePostToX()          2-4hr timer, autonomous X posting   │
│    ├─ maybeAnnounceWinners() 5min poll, 23:30-00:30 UTC only     │
│    ├─ maybeDistillMemory()    biweekly Sun 9-10am GMT+8           │
│    ├─ maybePostToMoltbook()   after 8:30am GMT+8, 35min cooldown  │
│    └─ maybeMoltbookEngage()   3-5hr timer, 9-23 GMT+8             │
│                                                                   │
│  Kill switch: agent/.memeya-x-enabled file                       │
│  Reboot-safe: timers init to Date.now() (no immediate post)      │
│  Boot cooldown: 5 min after start (prevent restart burst)        │
└──────────────────────────┬───────────────────────────────────────┘
                           │ dynamic import
                           ▼
┌──────────────────────────────────────────────────────────────────┐
│                    index.js (x_twitter skill)                     │
│                                                                   │
│  autoPost():                                                     │
│    1. gatherContext()           ←── x-context.js                 │
│    2. chooseTopic()             ←── weighted random              │
│    3. buildSystemPrompt()       ←── MEMEYA_PROMPT_BASE +         │
│                                     core values + longterm mem   │
│    4. extractBannedPatterns()   ←── dynamic anti-repetition      │
│    5. generateTweet()           ←── Grok API                     │
│    6. hard reject + boring-check ←── pattern + Grok gate         │
│    7. tweet via Twitter API                                      │
│    8. logPost()                 ←── diary entry                  │
│                                                                   │
│  distillMemory():               ←── biweekly journal distill     │
└──────────────────────────────────────────────────────────────────┘
         │                    │                    │
         ▼                    ▼                    ▼
┌──────────────┐  ┌────────────────────┐  ┌──────────────────┐
│ Grok API     │  │ Twitter API v2     │  │ MemeForge API    │
│ /chat/compl  │  │ POST tweet/reply   │  │ /api/memes/today │
│ /responses   │  │ GET search/recent  │  │ /hall-of-memes   │
│ (web search) │  │ (community search) │  │                  │
└──────────────┘  └────────────────────┘  └──────────────────┘
```

### 9.2 Context 與記憶架構

**三層記憶 (Three-tier Memory):**

| 層級 | 檔案 | 更新頻率 | 注入位置 |
|------|------|---------|---------|
| Core Values (不可變) | `memory/knowledge/memeya_values.md` | 人工管理 | System Prompt (`buildSystemPrompt`) |
| Long-term Memory | `memory/knowledge/memeya_longterm.md` | 雙週日 9-10am GMT+8 | System Prompt (`buildSystemPrompt`) |
| Short-term (日記) | `memory/journal/memeya/*.md` | 每次發文/留言 | User Prompt (topic prompt) |

**`gatherContext(baseDir)` 並行收集 User Prompt 資料來源:**

| 資料來源 | API/File | 逾時 | Fallback |
|---------|----------|------|----------|
| 今日梗圖 | `GET /api/memes/today` | 5s | `[]` |
| 歷史梗圖 (隨機 1) | `GET /api/memes/hall-of-memes?days=30&limit=30` | 5s | `null` |
| 近期 commits | `git log --since="12 hours ago"` | 5s | `[]` |
| Memeya 日記 | `memory/journal/memeya/` (最近 2 天, 2000 chars) | — | `''` |
| 最近 15 篇推文 | `memory/journal/memeya/` (最近 3 天) | — | `[]` |
| 粉絲留言 | Twitter API v2 `conversation_id` search (最近 3 篇, 各 top 3) | 5s/篇 | `[]` |
| 產品文件 | `docs/product.md` (for `feature_showtime`) | — | `''` |

> 注意: Core Values 和 Long-term Memory 不在 `gatherContext()` 中，而是透過 `buildSystemPrompt()` 注入到 System Prompt。

留言抓取依賴 `X_BEARER_TOKEN` (需 Basic tier)。Free tier 會收到 403，graceful 降級為空陣列。

### 9.3 話題選擇演算法

```javascript
// Phase 1: meme_spotlight — 強制發完今日 3 張梗圖 (各帶唯一 OG URL)
// Phase 2: 所有梗圖都發完後，進入 secondary topics

SECONDARY_TOPICS = [
  { id: 'personal_vibe',      weight: 25 },
  { id: 'crypto_commentary',  weight: 25 },
  { id: 'feature_showtime',   weight: 25 },
  { id: 'dev_update',         weight: 25 },
];

// community_response: 動態加入
// 有留言 → weight 20 (加入 pool)
// 留言 likes > 3 → weight 35 (eureka boost, 最高優先)
// 無留言 → 不加入 pool

// token_spotlight: 20% 機率觸發 (每日最多 1 篇)
```

**優先檢查 (Priority Override):**
- 若有新 git commits 且當日尚未發過 `dev_update` → 強制選擇 `dev_update`（每日最多 1 篇）
- Memeya 以建造者角度描述升級，而非列出技術 commit

**Fallback 規則:**
- `meme_spotlight` 無梗圖 → `personal_vibe`
- `dev_update` 無 commits → `personal_vibe`
- `feature_showtime` 無 product.md → `personal_vibe`
- `community_response` 無留言 → `personal_vibe`

**反重複:** 最近 3 篇若同話題，強制選不同話題。

### 9.4 推文生成流程

```
contextInput (string | {topic, prompt, ogUrl})
        │
        ▼
  ┌─────────────────────────────┐
  │ generateTweet(input, opts)   │
  │   opts: { detailed, noCharLimit } │
  │                              │
  │ 1. buildSystemPrompt(baseDir)│ ←── MEMEYA_PROMPT_BASE +
  │    (core values + longterm)  │     core values + longterm mem
  │                              │
  │ 2. loadRecentPosts(15)       │ ←── last 3 days journal
  │ 3. extractBannedPatterns()   │ ←── dynamic opener/phrase bans
  │                              │
  │ 4. Structured? → 用 prompt   │ (journal already in topic prompt)
  │    String?   → 載入 journal  │ (legacy 相容)
  │                              │
  │ 5. User prompt 組裝:         │
  │    topic prompt + anti-rep   │
  │    + recent 15 posts         │
  │    + banned patterns         │
  │                              │
  │ 6. crypto_commentary?        │
  │    → Grok /responses + web_search │
  │    其他?                     │
  │    → Grok /chat/completions  │
  │                              │
  │ 7. Strip Grok-generated URLs │
  │    Strip @mentions (Free tier)│
  │    Append canonical ogUrl    │ (meme-specific OG preview)
  └──────────────┬──────────────┘
                 │
                 ▼
  ┌─────────────────────────────┐
  │ Quality Gate (三層)          │
  │                              │
  │ Layer 1: Hard Reject         │
  │   "Yo degens"/"Degens," opener?  │
  │     → auto BORING            │
  │   "lava hammer" + 3+ recent? │
  │     → auto BORING            │
  │                              │
  │ Layer 2: Grok Check (grok-3-mini)│
  │   generic/repetitive/similar?│
  │     → BORING                 │
  │                              │
  │ Layer 3: Exception           │
  │   meme_spotlight + unique OG?│
  │     → SKIP (unique meme)    │
  │                              │
  │ OK → return tweet            │
  │ BORING → generate bored     │
  │   replacement action/speech  │
  │   → still posted to X       │
  └─────────────────────────────┘
```

### 9.5 社群互動循環

> **已移除:** X Community 回覆 (`communityEngage()`, `maybeCommunityEngage()`) — X API 不支援 Community 搜尋/回覆 (403 錯誤)。`community_response` 話題仍存在，但改為根據留言素材生成新推文，而非直接回覆 Community threads。

#### 自有推文留言巡邏 (gatherContext)

每次 `gatherContext()` 執行時：

1. 從日記讀取最近 3 篇有 URL 的推文
2. 從 URL 提取 tweet ID
3. 用 Twitter API v2 search `conversation_id:{tweetId} -from:AiMemeForgeIO` 抓取回覆
4. 依 likes 排序取 top 3
5. 寫入 Memeya 日記 (`## HH:MM:SS — Comment Review`)
6. 加入 context → 影響 `chooseTopic()` 權重

**日記格式:**
```markdown
## 14:30:00 — Comment Review
- On post: "just shipped tapestry comments..."
  - Reply (5 likes): this is so cool, love the meme forge concept
  - Reply (2 likes): when new memes dropping?
```

### 9.6 關鍵檔案

| 檔案 | 用途 |
|------|------|
| `agent/skills/x_twitter/index.js` | X posting skill: tools, generateTweet, autoPost, extractBannedPatterns, buildSystemPrompt |
| `agent/skills/x_twitter/x-context.js` | Context 收集 + 話題輪轉 + 留言抓取 + 日記記錄 |
| `agent/chat-mode.js` | Heartbeat → maybePostToX (2-4hr) + maybeAnnounceWinners (5min) + maybeCheckOwnerMentions (15min) + maybePostToMoltbook (after 8:30am) + maybeMoltbookEngage (3-5hr) + maybeDistillMemory (biweekly) |
| `agent/tg-community.js` | TG Community Bot: 雙群組 (EN + CN), respond to mentions/founder/keywords, share X posts to both groups, agent journal 作為知識庫 (murmur 已停用) |
| `agent/skills/moltbook/index.js` | Moltbook skill: autoPostMemes, engage, postIntroduction, ensureSetup + full Memeya context pipeline |
| `agent/skills/moltbook/moltbook-client.js` | Moltbook REST API client (auth, rate limit retry, verification challenge solver) |
| `agent/dashboard-server.js` | Dashboard API (prompt 查看, test-generate, send-post, 策略分析) |
| `agent/memeya-dashboard.html` | Memeya Dashboard UI (prompt 組成、pipeline、safeguards) |
| `memory/journal/memeya/*.md` | Memeya 日記 — 短期記憶 (推文/留言記錄) |
| `memory/knowledge/memeya_values.md` | Memeya 核心價值觀 — 不可變層 (人工編輯) |
| `memory/knowledge/memeya_longterm.md` | Memeya 長期記憶 — Grok 蒸餾層 (biweekly 自動更新) |
| `app/src/components/AgentPage.jsx` | Agent 公開個人檔案頁面 (`#agent` route) |
| `app/public/agent.json` | A2A Protocol 機器可讀 agent manifest |

### 9.7 環境變數 (Agent)

| 變數 | 用途 |
|------|------|
| `XAI_API_KEY` | Grok API (推文生成 + 品質審核 + 記憶蒸餾) |
| `X_CONSUMER_KEY` | Twitter OAuth 1.0a App Key |
| `X_CONSUMER_SECRET` | Twitter OAuth 1.0a App Secret |
| `X_ACCESS_TOKEN` | Twitter User Access Token (發推) |
| `X_ACCESS_SECRET` | Twitter User Access Secret |
| `X_BEARER_TOKEN` | Twitter App Bearer Token (搜尋/留言讀取) |
| `TELEGRAM_COMMUNITY_BOT_TOKEN` | TG Community Bot token — EN group (@memeya_bot) |
| `TELEGRAM_COMMUNITY_CHAT_ID` | TG Community group ID — EN (@MemeyaOfficialCommunity) |
| `TELEGRAM_COMMUNITY_CN_BOT_TOKEN` | TG Community Bot token — CN group |
| `TELEGRAM_COMMUNITY_CN_CHAT_ID` | TG Community group ID — CN |
| `MOLTBOOK_API_KEY` | Moltbook Agent API Key (posting, engagement) |
| `TAPESTRY_API_KEY` | Tapestry API Key (推文映射到 Tapestry) |
| `TAPESTRY_API_URL` | Tapestry API Base URL |
| `MEMEYA_TAPESTRY_PROFILE_ID` | Memeya 的 Tapestry profile ID |

### 9.8 Dashboard 端點

| 端點 | 方法 | 說明 |
|------|------|------|
| `/api/memeya/prompt` | GET | 查看組裝的 system prompt (base + values + longterm) + user prompt |
| `/api/memeya/journals` | GET | 最近 3 天 Memeya 日記 |
| `/api/memeya/values` | GET | Memeya 核心價值觀內容 |
| `/api/memeya/activity` | GET | 今日活動時間軸 |
| `/api/memeya/todos` | GET | TODO 列表 (parsed from `memory/TODO.md`) |
| `/api/memeya/test-generate` | POST | Generate Post: 完整 pipeline (支援 `purpose` 參數跳過自動話題 + 跳過 boring check，`noCharLimit`，`systemUpgrade` checkbox 標記為系統升級推文) |
| `/api/memeya/send-post` | POST | 發送推文到 @AiMemeForgeIO (body: `{ text, topic }`)。發文後自動重置自主發文計時器。 |
| `/api/memeya/x-post-status` | GET | 查詢 Heartbeat X Post 開關狀態 |
| `/api/memeya/x-post-toggle` | POST | 切換 Heartbeat X Post ON/OFF (file-based flag) |
| `/api/memeya/analyze` | POST | Grok 策略分析 |
| `/api/memeya/timers` | GET | Live timer state (read from `.timer-state.json`, written by heartbeat) |

---

## 10. Tapestry 社交整合

### 10.1 架構總覽

```
┌─────────────────────────────────────────────────────────────────┐
│                     Frontend (Vercel)                           │
│                                                                 │
│  CommentSection component                                       │
│    → GET /api/tapestry/comments/:memeId                         │
│    → POST /api/tapestry/comments                                │
└──────────────────────────────┬──────────────────────────────────┘
                               │
                               ▼
┌──────────────────────────────────────────────────────────────────┐
│                  Cloud Run API (Backend)                          │
│                                                                   │
│  routes/tapestry.js          services/tapestryService.js          │
│  ├ GET  /comments/:memeId    ├ findOrCreateProfile(wallet)        │
│  ├ POST /comments            ├ findOrCreateMemeContent(memeId)    │
│  └ GET  /comments/:id/count  ├ getComments(contentId)             │
│                              ├ createComment(profile, content, text)│
│  controllers/                ├ createVoteContent(profile, meme)    │
│  ├ userController: auto-     └ createAgentContent(text, props)     │
│  │   create profile on       │                                    │
│  │   new user                │                                    │
│  └ votingController: post    │                                    │
│      vote activity           │                                    │
└──────────────────────────────┼────────────────────────────────────┘
                               │
                               ▼
┌──────────────────────────────────────────────────────────────────┐
│                    Tapestry API                                    │
│                https://api.usetapestry.dev/v1/                    │
│                                                                   │
│  POST /profiles/findOrCreate    — Create/find user profile        │
│  POST /contents/create          — Create content nodes             │
│  GET  /comments?contentId=X     — Get comments for content        │
│  POST /comments                 — Create comment                   │
│                                                                   │
│  Auth: apiKey query parameter (server-side only)                  │
│  Blockchain: SOLANA, Execution: FAST_UNCONFIRMED                  │
└──────────────────────────────────────────────────────────────────┘
```

### 10.2 資料流

```
User votes on meme
    ├─ Firestore: save vote + tickets (existing)
    └─ Tapestry (non-blocking):
        ├─ findOrCreate user profile (if no tapestryProfileId)
        └─ create vote content ("Voted for X on AIMemeForge!")

User posts comment on meme
    ├─ Frontend: POST /api/tapestry/comments { memeId, walletAddress, text }
    └─ Backend:
        ├─ Look up tapestryProfileId (or create profile)
        ├─ Look up tapestryContentId for meme (or create content node)
        └─ POST comment to Tapestry API

Memeya posts on X
    ├─ Twitter API: tweet (existing)
    └─ Tapestry (non-blocking):
        └─ create content as Memeya agent profile
```

### 10.3 Firestore Schema 擴展

**users/{walletAddress}** — 新增欄位:
```javascript
tapestryProfileId: string | null  // Tapestry profile ID, set on first interaction
```

**memes/{memeId}** — 新增欄位:
```javascript
tapestryContentId: string | null  // Tapestry content node ID, set on first comment
```

### 10.4 環境變數

**Backend (Cloud Run):**
```
TAPESTRY_API_KEY=<from app.usetapestry.dev>
TAPESTRY_API_URL=https://api.usetapestry.dev/v1
```

**Agent (.env):**
```
TAPESTRY_API_KEY=<same key>
TAPESTRY_API_URL=https://api.usetapestry.dev/v1
MEMEYA_TAPESTRY_PROFILE_ID=<created once during setup>
```

### 10.5 關鍵檔案

| 檔案 | 用途 |
|------|------|
| `backend/services/tapestryService.js` | Tapestry API wrapper (profiles, content, comments) |
| `backend/routes/tapestry.js` | REST endpoints for frontend (comments CRUD) |
| `src/components/CommentSection.jsx` | Collapsed comments UI component |
| `src/services/memeService.js` | Frontend Tapestry comment API functions |
| `agent/skills/x_twitter/index.js` | Mirror X posts to Tapestry |
| `agent/dashboard-server.js` | Mirror manual posts to Tapestry |

---

## 11. 邀請朋友 (Referral ID System)

### 11.1 Referral ID 規格

| 參數 | 值 |
|---|---|
| 格式 | 3-8 字元 `[a-zA-Z0-9]`，區分大小寫 |
| 預設 | 6 字元隨機 (`crypto.randomBytes`, 62^6 ≈ 56B 組合) |
| 碰撞處理 | Retry up to 5 次 |
| 觸發 | 首次投票時自動生成 |
| Blocklist | 保留字過濾 (admin, test 等) |

### 11.2 Firestore Schema

**Collection: `referral_ids/{referralId}`**
```json
{
  "wallet": "52VCnQ...",
  "createdAt": "ISO timestamp",
  "isCustom": false
}
```

**User doc 增欄:** `referralId: "AbC123"`

### 11.3 自訂 ID 流程

Endpoint: `POST /api/users/:wallet/set-referral-id`

Firestore Transaction:
1. 驗證: 3-8 chars, `[a-zA-Z0-9]`, not in blocklist
2. 檢查唯一性: `referral_ids/{newId}` must not exist
3. 刪除舊 `referral_ids/{oldId}` doc
4. 建立新 `referral_ids/{newId}` doc
5. 更新 user doc `referralId` 欄位

### 11.4 連結解析 (Frontend `App.jsx`)

```
?ref= 參數長度判斷:
  3-8 字元 → GET /api/referral/:id → 解析為 wallet
  32-44 字元 → 直接作為 wallet (向後相容)
```

### 11.5 OG 分享

- **OG 圖片:** `invite-to-win-og.jpg`
- **路由:** Vercel Serverless Function (`app/api/ref/[id].js`)
- **行為:** 爬蟲讀 OG meta, 瀏覽器 JS 重導至 `/?ref={id}`

### 11.6 獎勵參數

| 參數 | 值 | 檔案 |
|---|---|---|
| `REFERRAL_BONUS_PCT` | 0.20 (被邀請者 +20%) | `rewardService.js` |
| `REFERRER_REWARD_PCT` | 0.10 (邀請人得 10%) | `rewardService.js` |
| `REFERRER_MEMEYA_THRESHOLD` | 50,000 $Memeya (菁英門檻) | `rewardService.js` |

### 11.7 關鍵檔案

| 檔案 | 用途 |
|---|---|
| `backend/controllers/votingController.js` | ID 生成 (首次投票時) |
| `backend/routes/users.js` | set-referral-id, set-referrer, referral-info endpoints |
| `src/components/ReferralTab.jsx` | 邀請連結 + 自訂 ID + 設定邀請人 UI |
| `app/api/ref/[id].js` | Vercel Serverless OG + 重導 |
| `backend/services/rewardService.js` | 邀請獎勵計算 |

---

## 12. Crossmint 錢包與獎勵分發

### 12.1 概述

Memeya 擁有自主管理的鏈上 Solana 錢包，透過 [Crossmint Agentic Wallet SDK](https://www.crossmint.com) (`@crossmint/wallets-sdk`) 實現。每日抽獎後自動分發 USDC 獎勵。

**流程:**
```
lottery_draw (23:55 UTC) → distributeRewards() → Crossmint SDK sendUsdc() → Firestore 記錄
        ↓ (within 5 min)
Agent heartbeat → maybeAnnounceWinners() → Tweet with meme image → PATCH xAnnounced: true
```

### 12.2 獎勵規則

| 受獎者 | 金額 | 選取方式 |
|--------|------|---------|
| 梗圖贏家 (最高票擁有者) | $3 USDC | lottery_draw 結果 |
| 幸運投票者 #1 | $2 USDC | Fisher-Yates 隨機 (排除贏家) |
| 幸運投票者 #2 | $1 USDC | Fisher-Yates 隨機 (排除贏家) |
| **每日合計** | **$6 USDC** | |

**$Memeya 門檻:** 所有受獎者 (贏家 + 投票者) 須持有 ≥ 10,000 $Memeya 才能獲得 USDC。贏家不足者仍獲得 Meme，但跳過 USDC 轉帳。

**餘額門檻:** 餘額 < $6 跳過分發 · 餘額 < $10 發送 TG 低餘額警報

**投票者選取 (`selectRandomVoters`):**
1. 查詢當日 votes (按 `timestamp` 範圍篩選，非 `date` 欄位)
2. 收集不重複投票者錢包 (排除贏家)
3. **每位投票者即時鏈上 RPC 查詢** $Memeya 餘額 (不依賴 Firestore 快取，避免用戶賣出後仍通過舊快取)
4. 更新 Firestore `users/{wallet}.memeyaBalance` 快取
5. Fisher-Yates 隨機洗牌選取 ≤ 2 位合格投票者

### 12.3 API 端點

| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/api/rewards/balance` | 錢包餘額 + 獎勵設定 (快取 10 分鐘) |
| GET | `/api/rewards/history?limit=N` | 近期分發紀錄 |
| GET | `/api/rewards/config` | 獎勵分發開關狀態 |
| POST | `/api/rewards/config` | Toggle 開關 (ON/OFF) |
| GET | `/api/rewards/:drawId` | 特定分發詳情 |
| GET | `/api/rewards/latest-unannounced` | 最近未公告的完成分發 (含 memeImageUrl, memeTitle) |
| PATCH | `/api/rewards/:drawId` | 更新分發紀錄 (含 `xAnnounced` 欄位) |
| POST | `/api/scheduler/trigger/reward_distribution` | 手動觸發獎勵分發 (支援 `body.date` 指定日期) |

### 12.4 模擬模式

Dashboard 可 toggle 獎勵分發 ON/OFF。OFF 時進入模擬模式:
- 計算金額、選取投票者，但 **不執行** USDC 轉帳
- Firestore 記錄 `status: 'simulated'`
- 發送 TG 報告 (完整模擬結果: 誰會收到多少)

### 12.5 Firestore Schema

**reward_distributions/{drawId}** (drawId = YYYY-MM-DD)
```javascript
{
  drawId: '2026-02-25',
  timestamp: '2026-02-25T23:55:00.000Z',
  status: 'completed' | 'simulated' | 'partial' | 'failed' | 'skipped_low_balance',
  balance: 60.00,
  winnerWallet: '4Bqy...SbMP',
  memeId: 'meme_xxx',
  transfers: [
    { type: 'winner', wallet: '4Bqy...', amount: 3, txSignature: '...' },
    { type: 'voter', wallet: '7Abc...', amount: 2, txSignature: '...' },
    { type: 'voter', wallet: '9Xyz...', amount: 1, txSignature: '...' }
  ],
  errors: [],
  randomVoters: ['7Abc...', '9Xyz...'],
  calculatedAmounts: { winner: 3, voter1: 2, voter2: 1, total: 6 },
  xAnnounced: true          // Agent 已在 X 發佈得獎公告
}
```

**reward_distributions/config**
```javascript
{
  rewardEnabled: true,     // false = 模擬模式
  updatedAt: '2026-02-25T14:30:00.000Z'
}
```

### 12.6 關鍵檔案

| 檔案 | 用途 |
|------|------|
| `backend/services/crossmintService.js` | Crossmint SDK wrapper (getWallet, balances, sendUsdc) |
| `backend/services/rewardService.js` | 獎勵計算、分發、模擬、TG 通知、鏈上餘額驗證 |
| `backend/services/solanaService.js` | Solana RPC: getMemeyaBalance (投票者資格即時驗證) |
| `backend/routes/rewards.js` | REST endpoints (balance, history, config, latest-unannounced) |
| `backend/services/schedulerService.js` | lottery_draw 後串接 distributeRewards (支援 dateOverride) |
| `scripts/crossmint-setup.js` | 一次性設定: 建立 Crossmint 錢包 |

### 12.7 X 得獎公告 (Winner Announcement)

Agent 在獎勵分發後自動發佈得獎推文 (含梗圖圖片)。

**流程:**
1. Agent heartbeat → `maybeAnnounceWinners()` (每 5 分鐘，僅 23:30-00:30 UTC)
2. `GET /api/rewards/latest-unannounced` → 取得最近未公告的分發紀錄
3. `buildWinnerAnnouncementText(dist)` → 動態生成推文 (遍歷 transfers[]，非硬編碼)
4. 取得梗圖圖片 → base64 → `POST /api/memeya/send-post` (dashboard server)
5. `PATCH /api/rewards/{drawId}` → 標記 `xAnnounced: true`
6. 重設 X 發文計時器避免碰撞 + TG devlog 通知

**Round 編號計算:**
```javascript
const ROUND_ONE_DATE = new Date('2026-02-26T00:00:00Z');
const roundNum = Math.max(1, Math.round((drawDate - ROUND_ONE_DATE) / 86400000) + 1);
```

**推文格式:**
```
AiMemeForge Round {N} Winners!

Meme Owner: 4Bqy...SbMP — Meme + $3 USDC
Lucky Voter: 7xKm...R2nP — $2 USDC
Lucky Voter: 9aFe...T4wQ — $1 USDC

Today's memes are live — vote now!
aimemeforge.io
```

推文含梗圖圖片附件，發佈後同步轉發至 EN + CN Telegram 群組。

**冪等性:** `xAnnounced: true` 防止重複公告 + 記憶體 `_announcedDrawIds` Set 防止 PATCH 失敗時同進程重複發推

### 12.8 環境變數

```
CROSSMINT_API_KEY=sk_production_...    # Crossmint Server-side API Key
MEMEYA_WALLET_LOCATOR=4BqywEbj...      # 錢包地址 (setup script 產生)
```

### 12.9 Dashboard 整合

- **Memeya Dashboard** (droplet): Wallet tab — 餘額、分發預覽、歷史、ON/OFF toggle
- **AiMemeForge 前端** (Vercel): Header 顯示 USDC 餘額，點擊開啟 Wallet 說明 modal
- Dashboard server proxy: `/api/memeya/wallet-*` → Cloud Run `/api/rewards/*`

---

## 13. 實作規劃

### Agent Memeya

- [x] X posting skill (`x_post`, `x_search`, `x_read_mentions`)
- [x] Memeya 人設 system prompt (`MEMEYA_PROMPT_BASE`)
- [x] Grok 生成推文 + boring-check 品質審核
- [x] 最近 15 篇推文反重複
- [x] Context 收集: 梗圖 API + git commits + journal + product.md (values 移至 system prompt)
- [x] 話題選擇: meme_spotlight 優先發完當日梗圖 → 4 種 secondary 等權重 + community_response 動態 + token_spotlight 20% 機率
- [x] `dev_update` 優先檢查 (有 commits 且當日未發 → 強制，每日最多 1 篇)
- [x] `feature_showtime` 話題 (載入 product.md，Grok 挑選功能介紹)
- [x] ~~`community_call`~~ 已移除
- [x] 自主發文 heartbeat (2-4hr, 全天候, 獨立於 heartbeat active hours gate)
- [x] Heartbeat X Post ON/OFF toggle (file-based kill switch: `agent/.memeya-x-enabled`)
- [x] `crypto_commentary` 用 Grok web search 即時新聞
- [x] 粉絲留言抓取 (Twitter API v2 conversation_id search)
- [x] 留言洞見寫入日記 + eureka boost 話題權重
- [x] OG 連結系統 (meme-specific URL, 自動清除 Grok URL, 程式化附上 canonical)
- [x] `meme_spotlight` + 未發過的梗圖 OG → 跳過 boring check
- [x] Memeya Dashboard: prompt 查看 + Generate Post + Send to X + 策略分析
- [x] Dashboard Generate Post: 支援 Purpose 手動模式 (無字數限制, X Premium)
- [x] `personal_vibe` 多樣化反思 (移除 2-5 字限制和固定 lava 參考)
- [x] Telegram 通知 (發文成功/失敗，獨立 try-catch 不影響主流程)
- [x] API key 缺失時 throw error (防止 debug 文字被發到 X)
- [x] 三層反重複系統: `extractBannedPatterns()` 動態分析 + hard reject 自動攔截 + Grok boring check 加強
- [x] 語言多樣性: `VARIETY IS KING` 規則 (forge 隱喻限 1/5、禁固定 opener、能量曲線變化)
- [x] 每個話題 `buildPrompt()` 個別抑制 forge/hammer 隱喻
- [x] `buildSystemPrompt()`: base prompt + core values + long-term memory 動態組裝
- [x] 三層記憶架構: Core Values (不可變) → Long-term Memory (biweekly 蒸餾) → Short-term (日記)
- [x] `maybeDistillMemory()`: 雙週日 9-10am Grok 分析日記 → 蒸餾成長期記憶
- [x] ~~X Community 互動~~: `maybeCommunityEngage()` 已移除 (X API 不支援 Community 回覆, 403)
- [x] Reboot-safe timers (初始化 `Date.now()` 而非 `0`，防重啟後立即發文)
- [x] Agent Profile 公開頁面 (`#agent` route + `agent.json` A2A manifest)
- [x] Dashboard 同步最新架構 (system/user prompt 分離, 三層記憶, safeguards 更新)
- [ ] Phase 2: 發文後 30 分鐘檢查互動指標，記錄到日記 → Grok 學習哪些話題有效

### Agent Memeya — Moltbook 社群

- [x] Moltbook skill: `moltbook_post`, `moltbook_comment`, `moltbook_feed`, `moltbook_search` tools
- [x] Moltbook API client: auth, rate limit retry, verification challenge solver (obfuscated math)
- [x] `autoPostMemes()`: 每日梗圖自動發布到 m/AiMemeForge (每日 1 篇，含歷史貼文去重機制防垃圾偵測)
- [x] `engage()`: 4 優先級自動互動 (reply → DM → upvote → comment)
- [x] `postIntroduction()`: 一次性自我介紹到 m/introductions
- [x] `ensureSetup()`: 自動建立 submolt、訂閱社群
- [x] 完整 Memeya context pipeline: `MEMEYA_MOLTBOOK_PROMPT` + Core Values + Long-term Memory + Journal
- [x] 所有生成使用 `grok-4-1-fast-reasoning` + template fallback
- [x] ~~`_tryCrossPost()`~~: cross-posting 已停用 (觸發 Moltbook 垃圾偵測)
- [x] 歷史貼文追蹤 (`memory/moltbook/post-history.json`): 傳入 AI prompt 避免重複用語結構
- [x] Heartbeat: `maybePostToMoltbook()` (after 8:30am GMT+8) + `maybeMoltbookEngage()` (3-5hr, 9-23 GMT+8)
- [x] Dashboard: Moltbook tab (posting templates, engagement rules, live state)
- [x] Dashboard: Timers tab (live timer state, auto-refresh 30s)
- [x] Moltbook profile: avatar uploaded, description updated, age reference removed
- [x] Moltbook 每日梗圖縮減至 1 篇 + cross-posting 停用 (防垃圾偵測)
- [x] Moltbook 歷史貼文去重 (`post-history.json`) — AI prompt 避免重複用語
- [ ] Week 1 monitoring: karma growth tracking, adjust engagement levels

### Phase 1: 每日抽獎 (Daily Lottery)

**目標**: 投票結算後，從所有「參與」的錢包中根據 tickets 加權隨機抽選 1 位贏家。

**Backend 核心邏輯:**
```javascript
async runDailyLottery() {
  // 1. 找到今日贏家梗圖 (最高票)
  const winnerMeme = todaysMemes.sort((a, b) =>
    (b.votes?.selection?.yes || 0) - (a.votes?.selection?.yes || 0)
  )[0];
  if (!winnerMeme || (winnerMeme.votes?.selection?.yes || 0) === 0) return;

  // 2. 查詢 lotteryOptIn=true 且 tickets > 0 的用戶
  // 3. 建立加權池 (每張 ticket = 1 個 entry)
  // 4. 隨機抽選 1 位
  // 5. 更新 meme.nftOwner + user.nftWins
  // 6. 所有參與者 weeklyTickets 歸零
}
```

**Frontend 變更:**
- ForgeTab: lottery opt-in/opt-out toggle
- Dashboard: 「My Wins」區域 (中獎紀錄 + Claim 入口)
- MemeModal / Gallery: 贏家 Owner badge

### Phase 2: NFT Claim & 鑄造

**目標**: 贏家 Claim → Arweave 上傳 → Metaplex pNFT 鑄造 (5% royalty 強制執行) → 用戶簽名。

**NFT 規格:**
- **Standard**: Metaplex Programmable NFT (pNFT)
- **Royalty**: 5% (強制執行，無法被交易所跳過)
- **Royalty 收款**: 平台錢包 (PLATFORM_WALLET)
- **Creator share**: Platform 5% royalty, Winner 100% ownership

**API:**
- `POST /api/nft/prepare-mint` — 準備 mint transaction (server-side)
- `POST /api/nft/confirm-mint` — 確認 mint 完成

**NFT Metadata (Metaplex pNFT):**
```json
{
  "name": "MemeForge #001 — When Solana Hits $200",
  "symbol": "MFORGE",
  "image": "https://arweave.net/xxx",
  "seller_fee_basis_points": 500,
  "attributes": [
    { "trait_type": "Rarity", "value": "Legendary" },
    { "trait_type": "Vote Count", "value": 42 },
    { "trait_type": "Generation Date", "value": "2026-02-18" }
  ],
  "properties": {
    "category": "image",
    "creators": [
      { "address": "PLATFORM_WALLET", "share": 100 }
    ]
  }
}
```

> `seller_fee_basis_points: 500` = 5% royalty。pNFT 的 RuleSet 確保所有交易所 (Magic Eden, Tensor 等) 強制執行此 royalty。

**防重複鑄造**: Firestore transaction guard，`minting_in_progress` 中間態。

**新增依賴:**
| 套件 | 用途 |
|------|------|
| `@metaplex-foundation/mpl-token-metadata` | pNFT metadata + RuleSet |
| `@metaplex-foundation/umi` | Metaplex framework |
| `@metaplex-foundation/mpl-token-auth-rules` | pNFT royalty enforcement |
| `@irys/sdk` | Arweave 上傳 |

**新增環境變數:** `ARWEAVE_WALLET_KEY`, `SOLANA_RPC_URL`, `PLATFORM_WALLET_KEYPAIR`

### ~~Phase 3: SPL Token 門檻~~ (已由 $Memeya 取代)

> 原計畫發行 $MFORGE 作為投票門檻。實際以 $Memeya token 實現: 持有 ≥10 tokens 獲 bonus tickets, ≥10K 參與 USDC 獎勵, ≥50K 菁英邀請獎勵。詳見 §12 (Crossmint 錢包) 和 §11 (Referral)。

---

## 14. 開發進度 Roadmap

### 基礎設施

- [x] GCP 專案建立 (web3ai-469609)
- [x] Firestore 資料庫設定
- [x] Firestore Composite Index (memes: status, type, generatedAt)
- [x] GCS Bucket 建立 (memeforge-images-web3ai, Uniform Access)
- [x] Cloud Run 部署 (memeforge-api, asia-southeast1)
- [x] Vercel Frontend 部署
- [x] Cloud Scheduler 設定 (end_voting, daily_cycle)
- [x] DigitalOcean Droplet (165.22.136.40) 開發環境
- [x] GitHub Repo 同步流程
- [x] 環境變數管理 (.env 分離: backend, frontend, agent)
- [x] .gitignore 防止 secrets 洩漏

### AI 梗圖生成

- [x] Gemini 3 Pro Image 整合
- [x] Grok Imagine Image Pro 整合 (xAI Images API)
- [x] 多模型隨機選擇 — 每張梗圖隨機分配 Gemini 或 Grok，metadata.aiModel 標記
- [x] Grok API 新聞分析 (48hr 時間窗, A/B/C 三分類含 Macro/地緣/AI)
- [x] 每日自動生成 3 張梗圖
- [ ] 更多模型支援 (ChatGPT/DALL-E)
- [x] GCS 圖片上傳 (修復 Uniform Access 衝突)
- [x] 梗圖品質篩選 (幽默度、病毒潛力)
- [x] 內容來源: Twitter/X, CoinDesk, Reddit
- [x] 生成失敗重試/錯誤處理
- [x] V2: 15 種模板格式 + 策略+敘事層 + ≤6 字文案 + LLM 品質評估 (≥82/100)
- [x] V3: 10 種 V1 藝術風格 per-meme 選擇 (7 天不重複)
- [x] V3: Mode A (Template + Art Style) — 2 張/天
- [x] V3: Mode B (Art-first Original) — 1 張/天，自由構圖
- [x] V3: metadata 擴充 (artStyleId, artStyleName, artStyleNftTier, generationMode)

### 投票系統

- [x] 投票 API (POST /api/voting/vote)
- [x] 防重複投票 (Firestore 查詢)
- [x] 頻率限制 (Rate Limiting)
- [x] Ticket 獎勵 (base 1-10 + streak bonus, max +10 + $Memeya token bonus)
- [x] 連勝追蹤 + 獎勵 (streakDays → bonus tickets)
- [x] 投票統計即時更新
- [x] 投票結算邏輯 (end_voting)
- [x] 每日贏家選出 (最高票)
- [x] 稀有度計算 (Common/Rare/Legendary)

### 前端 UI

- [x] Privy 認證整合 (Google 登入 + OKX/Phantom 優先 + 自動偵測 Solana 錢包)
- [x] Embedded wallet 私鑰匯出 (useExportWallet, 設定選單)
- [x] Dashboard 設定選單 (gear icon dropdown: 地址複製、私鑰匯出、How It Works、外部連結、Sign Out)
- [x] 每日梗圖展示 (ForgeTab)
- [x] 投票界面 + 即時統計
- [x] 梗圖大圖 Modal (MemeModal)
- [x] Modal 內投票功能
- [x] Hall of Memes (GalleryTab)
- [x] 用戶 Dashboard (tickets, streak)
- [x] 響應式設計 (Mobile compatible)
- [x] $Memeya 餘額顯示 (header + Tickets tab bonus card, backend proxy via Helius RPC)
- [x] Share to X (Twitter) 按鈕
- [x] Copy Link 按鈕
- [x] OG Card 動態預覽圖

### 排程與自動化

- [x] Cloud Scheduler end_voting job
- [x] Cloud Scheduler daily_cycle job
- [x] Scheduler 查詢對齊 (與前端 query 一致)
- [x] Missing document 容錯處理
- [x] Generation 錯誤偵測 (statusCode >= 400)
- [x] 排程日誌 (scheduler_logs collection)
- [x] Cloud Scheduler lottery_draw job (每日 23:55 UTC)

### Phase 1: 每日抽獎

- [x] `lotteryOptIn` 欄位加入 users collection
- [x] `runDailyLottery()` 後端邏輯
- [x] 加權隨機抽選演算法
- [x] 參與者 tickets 歸零
- [x] 不參與者 tickets 保留
- [x] `nftOwner` 欄位更新 meme document
- [x] `nftWins` 陣列更新 user document
- [x] lottery_draw Cloud Scheduler job 建立 (每日 23:55 UTC)
- [x] Frontend: opt-in/opt-out toggle
- [x] Frontend: 「My Wins」Dashboard 區域 (中獎紀錄 + Claim 入口)
- [x] Frontend: Gallery #1 最高票 badge + Top Voted 篩選 (區別於 Winners tab 抽獎人類贏家)
- [x] 邊界處理: 0 參與者、1 參與者、0 投票

### Tapestry 社交整合

- [x] `tapestryService.js` API wrapper (profiles, content, comments)
- [x] `routes/tapestry.js` 評論 CRUD 端點
- [x] `server.js` 註冊 `/api/tapestry` 路由
- [x] `userController.js` 新用戶自動建立 Tapestry profile
- [x] `votingController.js` 投票後發佈活動到 Tapestry
- [x] `CommentSection.jsx` 折疊式評論 UI 組件
- [x] `memeService.js` 前端 Tapestry 評論 API 函數
- [x] MemeModal + GalleryTab 整合評論區
- [x] Agent `autoPost()` 映射推文到 Tapestry
- [x] Dashboard `send-post` 映射手動推文到 Tapestry

### 邀請朋友 (Referral)

- [x] Referral ID 自動生成 (6 字元, 首次投票)
- [x] 自訂 ID (3-8 字元, Firestore transaction)
- [x] `set-referrer` 接受 ID 或 wallet (自動判斷)
- [x] OG 圖片 + Vercel Serverless 重導 (`app/api/ref/[id].js`)
- [x] 菁英用戶獎勵: 邀請人 10% + 被邀請者 +20%
- [x] `#invite` hash route 快捷入口
- [x] Frontend ReferralTab: 連結、自訂 ID、設定邀請人
- [x] i18n (EN / 简体中文 / 繁體中文)
- [x] Wiki 頁面 Referral 章節

### 近期更新 (2026-02-28 ~ 03-02)

- [x] X 推文同步至 EN + CN Telegram 群組
- [x] Dashboard Generate Post: System Upgrade checkbox + 有 purpose 跳過 boring check + 發文後重置計時器
- [x] 5 分鐘啟動冷卻 (防重啟連發)
- [x] meme_spotlight 投票 CTA「Vote for this Meme 👇」
- [x] 得獎公告「AiMemeForge Round N Winners!」格式 (Round 1 = 2026-02-26)
- [x] 新聞時間窗擴展至 48hr + Macro/地緣衝突/AI 新聞分類
- [x] Wiki 頁面 (新手指南、How-to、代幣經濟、Roadmap、FAQ)
- [x] USDC 獎勵專區 (My Wins 頁籤)

### 近期更新 (2026-03-03 ~ 03-07)

- [x] Solana 付費梗圖生成 (Create Tab): SOL / $Memeya / USDC 三種支付
- [x] My Memes 畫廊 + 詳細檢視 (分享/複製連結/刪除)
- [x] DELETE /api/memes/:id 擁有權驗證刪除
- [x] Ticket 分配透明度圓餅圖 (TicketPieChart + hasTicketSnapshot)
- [x] Homepage Memeya Activity Ticker
- [x] Tab 間 CTA 導航橫幅統一設計
- [x] Winners tab Votes → Tickets 欄位修正
- [x] Agent 新聞搜尋改為確定性 heartbeat (4hr cooldown)
- [x] Telegram bridge #news 手動觸發新聞搜尋
- [x] 自動滾動至頁頂 on tab navigation

### 獎勵分發修復與強化 (2026-02-27)

- [x] **Bug fix:** `selectRandomVoters` 查詢 `date` 欄位改為 `timestamp` 範圍查詢 (votes collection 無 `date` 欄位，導致從未選出幸運投票者)
- [x] **Bug fix:** 投票者 $Memeya 餘額改為即時鏈上 RPC 查詢 (不再依賴可能過期的 Firestore 快取，防止已賣出用戶誤入選或持有者被排除)
- [x] `GET /api/rewards/latest-unannounced` 端點 (Agent 輪詢用)
- [x] `PATCH /api/rewards/:drawId` 支援 `xAnnounced` 欄位
- [x] `POST /api/scheduler/trigger/reward_distribution` 支援 `body.date` 指定日期重跑
- [x] Agent `maybeAnnounceWinners()` — 得獎公告自動發推 (含梗圖圖片，5 分鐘輪詢，僅 23:30-00:30 UTC)
- [x] Agent `buildWinnerAnnouncementText()` — 動態推文生成 (遍歷 transfers[]，處理跳過/無投票者邊界)
- [x] Dashboard Timer tab 顯示 Winner Announcement 計時器

### Agent 公開個人檔案

- [x] `app/public/agent.json` — A2A Protocol 機器可讀 manifest
- [x] `app/src/components/AgentPage.jsx` — 全頁 Agent Profile (人設、能力、技術棧、語錄)
- [x] `App.jsx` `#agent` hash route (公開頁面, 不需認證)
- [x] Footer "Meet Memeya" 連結

### Phase 2: NFT Claim & 鑄造

- [ ] `POST /api/nft/prepare-mint` API
- [ ] `POST /api/nft/confirm-mint` API
- [ ] Arweave 圖片上傳 (Irys SDK)
- [ ] Metaplex NFT metadata 建構
- [ ] Solana mint transaction 建構
- [ ] Frontend: Claim NFT 按鈕
- [ ] Frontend: Claim 流程 UI (簽名、等待、完成)
- [ ] Firestore transaction guard (防 double mint)
- [ ] Meme status 'minted' 終態保護
- [ ] Solana Explorer 可查看 NFT

### Phase 3: SPL Token 門檻

~~Phase 3 已由 $Memeya token 取代 — 見上方~~

---

*最後更新: 2026-03-07*

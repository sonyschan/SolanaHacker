...
│   ├── chat-mode.js           # Heartbeat: maybePostToX + maybeCommunityEngage + maybePostToMoltbook + maybeMoltbookEngage + maybeDistillMemory + maybeTgCommunityTick
...
```
┌──────────────────────────────────────────────────────────────────┐
│                    chat-mode.js (Heartbeat — 30min interval)      │
│                                                                   │
│  doHeartbeat()                                                    │
...
│    ├─ maybeCommunityEngage()  2-4hr timer, X community replies    │
...
│                                                                   │
│  communityEngage():             ←── X community replies          │
...
BASE_TOPICS = [
  ...
  { id: 'crypto_commentary',  weight: 20 },
...
// x_community_engage: 獨立 timer (2-4hr), 不在 topic pool 中
// 直接由 maybeCommunityEngage() 呼叫 communityEngage()
```
...
#### X Community 互動 (maybeCommunityEngage)

Heartbeat 獨立 timer (2-4hr)，scope 限定在 X Community：

```
範圍:
  X Community (ID: 2025765989582004365) → 巡邏 + 回覆留言
  @AiMemeForgeIO 時間軸 → 巡邏 only (不回覆，只收集素材)

流程:
  1. communityEngage() — 搜尋社群最新帖子
  2. Grok 分析帖子內容，判斷是否值得回覆
  3. 生成自然風格回覆 (避免機器人感)
  4. 發布回覆 → 記錄到日記
```

Timer 與 `maybePostToX` 獨立運行，互不阻塞。
...
| `agent/chat-mode.js` | Heartbeat → maybePostToX (2-4hr) + maybeCommunityEngage (2-4hr) + maybePostToMoltbook (after 8:30am) + maybeMoltbookEngage (3-5hr) + maybeDistillMemory (biweekly) + maybeTgCommunityTick |
| `agent/tg-community.js` | TG Community Bot: listen to @MemeyaOfficialCommunity, respond to mentions/founder/keywords, murmur when quiet, share X posts |
...
| `TELEGRAM_COMMUNITY_BOT_TOKEN` | TG Community Bot token (@memeya_bot) |
| `TELEGRAM_COMMUNITY_CHAT_ID` | TG Community group ID (@MemeyaOfficialCommunity) |
...
### 11.7 X 得獎公告 (Winner Announcement)
...
- [x] X Community 互動: `maybeCommunityEngage()` (2-4hr timer, 限 Community 內回覆)
...
- [x] `autoPostMemes()`: 每日梗圖自動發布到 m/AiMemeForge (35min cooldown)
...
- [x] `_tryCrossPost()`: AI 生成 cross-post 到 m/general, m/aiart, m/memes

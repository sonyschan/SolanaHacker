...
**🚧 In Progress:** Collab Memes 聯名梗圖服務 (見下方) · NFT Claim & 鑄造 (Metaplex pNFT, 5% royalty, Arweave) · Virtuals ACP Marketplace 上架準備 · Dexter Marketplace 上架
...
### Collab Memes 聯名梗圖 (開發中)

慶祝合作/整合的聯名梗圖生成服務。兩個項目的身份融合成一張梗圖，附帶建議推文。

**服務擴張定位**: 從單一項目梗圖 → 雙項目聯名梗圖，開拓 B2B 場景 (項目方合作公告、上架慶祝、整合發佈)。

**Wizard 流程** (Dashboard Lab Tab):
1. **輸入 URL** — 合作方 X 帳號或網站 URL → 自動抓取 profile (名稱/頭像/簡介)
2. **Profile 預覽** — 雙方 profile 卡片 + LLM 建議語調 (hype/flex/wholesome/chaos)
3. **生成** — 選擇合作類型 + 填寫標題 → AI 生成聯名梗圖 + 建議推文

**合作類型**: `integration` (技術整合) · `listing` (上架) · `partnership` (戰略合作) · `launch` (聯合發佈) · `migration` (遷移)

**語調**: `hype` (慶祝 LFG) · `flex` (炫耀實力) · `wholesome` (溫馨社群) · `chaos` (荒誕跨界)

**API 端點**: `POST /api/memes/generate-collab` · $0.15 USDC · x402 + Lab key 雙軌認證

**生成管線**: 複用 V3 管線 (藝術風格 → 策略 → 敘事 → 品質評估/重試 → 圖片生成 → 標題/描述/標籤) + 聯名專屬 prompt (雙項目身份、語調指令、合作類型語境) + 建議推文 (附 @mention)

**產出**: 梗圖圖片 + 標題 + 描述 + 標籤 + 建議推文 + 完整 metadata (品質分數、策略、敘事、藝術風格)。存入 Firestore `type: 'collab'`。

**Phase 2 (規劃中)**: Create Tab 公開 UI — 用戶付費生成聯名梗圖、分享到 X、存入 My Memes。
...
**獎勵分配** (每日 23:55 UTC):
- 梗圖贏家 (最高票): 國庫 10% USDC
- 幸運投票者 1 (隨機): 國庫 7% USDC
- 幸運投票者 2 (隨機): 國庫 4% USDC
...
| `/api/memes/generate-collab` | POST | $0.15 USDC | 聯名梗圖生成，雙項目合作慶祝梗圖 + 建議推文 |

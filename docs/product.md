...
**核心價值**: AI 每日創作 · 社群民主驗證品質 · 按需 API 服務 · 免費參與生態 · x402 微支付商務
...
- x402 Meme API — 按次付費 USDC on Base (Rate $0.005 / Generate $0.10 / Catalog Free)
...
**🚧 In Progress:** NFT Claim & 鑄造 (Metaplex pNFT, 5% royalty, Arweave) · OG Card 分享 (後端 OG 圖片服務已建，前端整合中) · Virtuals ACP Marketplace 上架準備
...
| 端點 | 方法 | 價格 | 說明 |
|------|------|------|------|
| `/api/memes/rate` | POST | $0.005 USDC | AI 品質評分，回傳 score/grade/pass/suggestions |
...
**支付協議**: x402 (HTTP 402 Payment Required) · USDC on Base (EIP-155:8453) · Coinbase CDP Facilitator

**客戶端整合**: `npm install @x402/fetch @x402/evm viem` — 使用 `wrapFetchWithPayment` 自動處理 402 challenge → 簽名 → 付款 → 取得結果。

**收款錢包**: `0xba646262871d295DeAe3062dF5bbe31fcc5841b8` (Crossmint Smart Wallet on Base)
